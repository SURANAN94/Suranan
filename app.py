...
st.title("🔍  เครื่องมือรวมข้อมูล Excel จัดทำโดย นาย สุรนันท์ ยามดี โรงพยาบาลส่งเสริมสุขภาพตำบลบ้านโนนค้อ ")

st.markdown(
    """
**วิธีใช้**  
1. อัปโหลด **F1** (ไฟล์ข้อมูลหลัก)  
2. อัปโหลด **F2** (ไฟล์ที่ต้องการดึงข้อมูลจาก)  
3. เลือกคอลัมน์คีย์ที่ใช้จับคู่ (เช่น `PID`) ของแต่ละไฟล์  
4. เลือก **คอลัมน์ผลลัพธ์จาก F2** ได้ทีละหลายคอลัมน์  
5. กดปุ่ม **ดึงข้อมูลและดาวน์โหลด** เพื่อรับไฟล์ผลลัพธ์
"""
)
